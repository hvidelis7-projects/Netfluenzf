...
const VerifyEmail: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-5 py-20">
      <div className="w-full max-w-md bg-white/60 backdrop-blur-xl border border-white/50 p-8 rounded-[2.5rem] shadow-2xl">
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full py-4 button-brand rounded-xl text-xs font-black uppercase tracking-widest shadow-xl active:scale-95 transition-transform disabled:opacity-70"
          >
            {busy ? 'Please wait…' : "I've verified — continue"}
          </button>
          <button
            type="button"
            disabled={busy}
            onClick={() => void handleResend()}
            className="w-full py-3 rounded-xl text-xs font-bold uppercase tracking-widest border-2 border-gray-200 bg-white/70 text-gray-800 hover:border-[#FF5500]/40 transition-colors disabled:opacity-70"
          >
